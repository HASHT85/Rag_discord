...
async def on_ready() -> None:
    ...
    # ── Nettoyer TOUS les doublons locaux (guild-level) pour ne garder QUE les commandes globales ──
    for guild in bot.guilds:
        try:
            bot.tree.clear_commands(guild=guild)
            await bot.tree.sync(guild=guild)
            logger.info("   🧹 Doublons locaux supprimés sur %s", guild.name)
        except Exception as exc:
            logger.warning("   ⚠️ Erreur nettoyage guild %s : %s", guild.name, exc)
